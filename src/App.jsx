...
</head><body id="corpo"></body>
<script>
(function(){
  const PAPEL = "${PAPEL}";
  const html  = \`${linhasContrato.replace(/`/g,"\`").replace(/\${/g,"\${")}\`;
  const corpo = document.getElementById("corpo");

  // Cria um div oculto para medir o conteúdo
  const medidor = document.createElement("div");
  medidor.style.cssText = "position:fixed;top:0;left:0;width:174mm;visibility:hidden;font-family:'Times New Roman',serif;font-size:11pt;z-index:-1";
  medidor.innerHTML = html;
  document.body.appendChild(medidor);

  // Divide em páginas de altura 215mm (297 - 38 topo - 44 rodapé)
  const ALTURA_PX = medidor.offsetHeight;
  const PAGINA_MM = 215;
  const PAGINA_PX = PAGINA_MM * (96/25.4);
  const TOTAL_PAGS = Math.ceil(ALTURA_PX / PAGINA_PX) || 1;
  ...
  for(let i=0;i<TOTAL_PAGS;i++){
    ...
    cont.style.top = (38 - i * PAGINA_MM) + "mm";
